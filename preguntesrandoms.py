...
with open('C:\\Users\\bosch\\Desktop\\Preguntes random\\log.txt', 'a', encoding='utf-8') as c:     #crea els 2 arxius si no estan
    pass
with open('C:\\Users\\bosch\\Desktop\\Preguntes random\\respostes.txt', 'a', encoding='utf-8') as v:
    pass
...
def printandlog(s):           #la s es refereix a la variable random line mes endevant/ funció per guardar i per escriure
    print(s)
    with open('C:\\Users\\bosch\\Desktop\\Preguntes random\\log.txt', 'a', encoding='utf-8') as f: #obrir document log per escriure tmb/ la n fica un espai extra??? 
        f.write(s + '\n') # / la s es torna a referir a la linea random de mes endevant, pertant escriu la linea random + el espai...........^^
    x = input('Resposta:')
    with open('C:\\Users\\bosch\\Desktop\\Preguntes random\\respostes.txt', 'a', encoding='utf-8') as k:
        k.write(x + '\n')

while True:  #mentres quedin preguntes NO a log
    with open('C:\\Users\\bosch\\Desktop\\Preguntes random\\log.txt', 'r', encoding='utf-8') as f:           # assigna el document a la variable logdoc per compararles
        logdoc = f.read()
    ...
    if not lines_not_in_logdoc:      # si totes les lineas de log doc (log.txt) estan a preguntes.txt escriu JA ESTA   log = preguntes   
        ...
        with open('C:\\Users\\bosch\\Desktop\\Preguntes random\\log.txt', 'w', encoding='utf-8') as b:
            pass  
        with open('C:\\Users\\bosch\\Desktop\\Preguntes random\\respostes.txt', 'w', encoding='utf-8') as a:
            pass  
        break
